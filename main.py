...
def two_pivot_block_quicksort(A, left, right):
    if right <= left:
        return
    ...
    two_pivot_block_quicksort(A, left, i - 1)
    two_pivot_block_quicksort(A, i, j - 1)
    two_pivot_block_quicksort(A, j + 1, right)
...
new_limit = 2000000000  # Sesuaikan dengan kebutuhan Anda
...
def analyze_sorting_performance(dataset_size, variation, output_file):
    ...
    two_pivot_block_quicksort(data.copy(), 0, len(data) - 1)
    end_time = time.time()
...
output_folder = "sorting_results"  # Nama folder untuk menyimpan hasil
...
new_limit = 1000000  # Sesuaikan dengan kebutuhan Anda
sys.setrecursionlimit(new_limit)
